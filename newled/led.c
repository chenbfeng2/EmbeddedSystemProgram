	...
	int databuild(void)
{
	...
	int previousTime1=8900;
	int previousTime2=8900;
	int previousTime3=8900;
	int previousTime4=8900;
		...
		case 2:
			...
			for (y=0;y<4;y++)
				...
				for (z=0;z<timeInterval-1;z+=2)
					{if (y==2)
										{
											dataout[lineNum+y].r[dataout[lineNum+y].wrtpt+1]=4;
											dataout[lineNum+y].wrtpt++;
										}
										else{
												dataout[lineNum+y].r[dataout[lineNum+y].wrtpt+1]=0;
...
for (k=0;k<2370;k++)
{
	...
	for (q=0;q<4;q++)
	{
		...
		for (i=0;i<60;i++)
			...
			if (q==0&&(i>54&&i<60))
			{
				strcat(buffer,bound2);
			}

			else if (i==54)
			{
				strcat(buffer,bound1);
			}
					...
					else if (dataout[q].r[(dataout[q].rdpt)+i]==4)
												{
												strcat(buffer,bound3);
													}
					else
						strcat(buffer,empty);
					}
		...
		{if (q==0&&(i>54&&i<60))
		{
		...
		else if (i==54)
			{
				strcat(buffer,bound1);
							...
							else if (dataout[q+4].r[(dataout[q+4].rdpt)+i]==4)
							{
							strcat(buffer,bound3);
								}
							else
		...
		{if (q==0&&(i>54&&i<60))
		{
		...
		else if (i==54)
			{
				strcat(buffer,bound1);
							...
							else if (dataout[q+8].r[(dataout[q+8].rdpt)+i]==4)
														{
														strcat(buffer,bound3);
															}
							else
		...
		{if (q==0&&(i>54&&i<60))
		{
		...
		else if (i==54)
			{
				strcat(buffer,bound1);
							...
							else if (dataout[q+12].r[(dataout[q+12].rdpt)+i]==4)
														{
														strcat(buffer,bound3);
															}
							else
...
int *pausecheck(void)
{
	...
	while(programend==0)
	{
		...
		if(is_high(8,11))  /* Check if in is high (i.e. button pressed) */
			...
			{
			       pthread_mutex_lock( &condition_mutex );
			       if(is_low(8,11))   /* Check if in is high (i.e. button pressed) */
			       {
			       			stop=0;
...
int main()
{
   ...
   // pthread_create( &thread4, NULL, &patternPrint, NULL);
    if(played==1&&programend==1)
    {
    	printf("Good finish! your final score is %ld\n",finalmark);
    	printf("Do you wanna play again? Y(Down)/N(Right)\n");
    }
    else
    {
    	printf("The game is ended, do you want to start over?\n");
    }
